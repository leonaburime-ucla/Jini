 ...
 * `(app: unknown, services) => void` in `@jini/core` so the kernel never depends on Express)
 * onto a concrete Express app. This is the piece of extraction-plan.md §3's
 ...
 * `Pack.cli`, so both transports call one shared app-service (extraction-plan.md §2.3).
 *
