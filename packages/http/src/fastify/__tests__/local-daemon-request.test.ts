...
/** Mirrors the shape `validateLocalDaemonRequest`/`requireLocalDaemonRequest` actually read off a
 * `FastifyRequest`: `.socket.remoteAddress` and plain `.headers` (no Express-style `.get()`). */
function makeRequest(opts: { remoteAddress?: string; host?: string; origin?: string }) {
  return {
    socket: { remoteAddress: opts.remoteAddress },
    headers: { host: opts.host, origin: opts.origin },
  } as any;
}
...
describe('validateLocalDaemonRequest', () => {
  ...
  it('accepts a duplicate-header (array-shaped) Host header, taking the first value', () => {
    const req = { socket: { remoteAddress: '127.0.0.1' }, headers: { host: ['127.0.0.1:4173', 'other'] } } as any;
    expect(validateLocalDaemonRequest(req)).toEqual({ ok: true, origin: null });
  });
...
describe('requireLocalDaemonRequest', () => {
  ...
  it('rejects a non-loopback request with 403 and does not call done()', () => {
    const req = makeRequest({ remoteAddress: '10.0.0.5', host: '127.0.0.1:4173' });
    const reply = makeReply();
    const done = vi.fn();

    requireLocalDaemonRequest(req, reply as any, done);
  ...
  it('sets CORS headers scoped to the validated Origin and calls done() on a valid request', () => {
    const req = makeRequest({ remoteAddress: '127.0.0.1', host: '127.0.0.1:4173', origin: 'http://127.0.0.1:4173' });
    const reply = makeReply();
    const done = vi.fn();
    ...
    expect(reply.header).toHaveBeenCalledWith('Access-Control-Allow-Origin', 'http://127.0.0.1:4173');
    expect(reply.header).toHaveBeenCalledWith('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
    expect(reply.header).toHaveBeenCalledWith('Access-Control-Allow-Headers', 'Content-Type');
    expect(reply.header).toHaveBeenCalledWith('Access-Control-Max-Age', '600');
    expect(done).toHaveBeenCalledOnce();
  });

  it('omits Access-Control-Allow-Origin when the request carried no Origin header, but still calls done()', () => {
    const req = makeRequest({ remoteAddress: '127.0.0.1', host: '127.0.0.1:4173' });
    const reply = makeReply();
    const done = vi.fn();
    ...
    expect(reply.header).not.toHaveBeenCalledWith('Access-Control-Allow-Origin', expect.anything());
    expect(done).toHaveBeenCalledOnce();
  });
});
