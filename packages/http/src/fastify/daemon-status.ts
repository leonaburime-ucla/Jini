 ...
 * data plus framework-agnostic `parse`/`handle` functions, see `../express/daemon-status.ts`'s
 * own doc for their full provenance) are intentionally NOT duplicated here: they never reference
 * Express at runtime (`../express/daemon-status.ts` only imports `type { Express }` for its own
