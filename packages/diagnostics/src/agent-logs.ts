 ...
 *
 * These are a host daemon's capture of each agent CLI's own stdout/stderr/stream
 * for a run — the single richest, agent-agnostic diagnostic signal for "the
 * run failed / produced nothing". Shared so both an HTTP export and a desktop
...
export interface AgentCliLogOptions {
  ...
  dataDir?: string | null;
  /**
   * Effective AMR-style OpenCode home (the resolved `OPENCODE_TEST_HOME`).
   * Callers should pass the value a real run would use so this honors a
   * user override; when omitted this falls back to the default
   * `<dataDir>/amr/opencode-home`.
   */
  amrOpenCodeHome?: string | null;
  /**
   * Effective Claude config home (`CLAUDE_CONFIG_DIR`). Honors a user
   * override; falls back to `~/.claude`.
   */
  claudeConfigDir?: string | null;
  /**
   * Effective Codex home (`CODEX_HOME`). Honors a user override; falls
   * back to `~/.codex`.
   */
...
/**
 * Collect the native on-disk logs of the coding-agent CLIs a host application
 * drives: Claude Code, Codex, OpenCode, and an AMR-style OpenCode runtime.
 * These complement the per-run `events.jsonl` capture with the CLI's own
 * internal diagnostics (provider errors, retries, auth state) that never
 * reach the daemon's stream.
 *
 * Deliberately excluded: secret-bearing files. This only sweeps known *log*
...
export async function buildAgentCliLogSources(options: AgentCliLogOptions): Promise<LogSource[]> {
  ...
  // same fix as the AMR-style runtime's `OPENCODE_TEST_HOME` so relocated
  // homes are not missed.
  ...
  // An AMR-style runtime drives a private OpenCode under OPENCODE_TEST_HOME;
  // its session logs land under that home's XDG data dir, where provider
  // failures are recorded. Prefer the effective home a real run resolves
  // (honoring a user override) and fall back to the default
  // `<dataDir>/amr/opencode-home` so the bundle does not silently drop the
  // very logs this is meant to surface.
