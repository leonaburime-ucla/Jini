...
// `validateSchedule`/`validateTarget` (real @jini-ai/daemon logic, unmocked in every other test in this
...
vi.mock('@jini-ai/daemon', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@jini-ai/daemon')>();
