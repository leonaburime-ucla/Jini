...
checked into the the host platform repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any the host platform agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for the host platform and
...
2. Replace the contents of this directory with the upstream snapshot,
   ...
   inside `SKILL.md`'s `> **the host platform integration.**` blockquote.
...
flow (including how the host platform behaves when Codex is not installed).
