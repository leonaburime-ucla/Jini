...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in the host platform.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current the host platform scope

the host platform currently ships this entry as discovery metadata only. If this `SKILL.md`
...
This catalogue entry advertises the skill in the host platform so the agent
...
If those upstream files are not installed, explain that the host platform only has the
catalog entry for this skill and ask whether to continue with the host platform's
