...
Use the unified the host platform media dispatcher. Do not call provider APIs or
...
out=$("$AGENT_NODE_BIN" "$AGENT_BIN" media generate \
...
while [ -n "$task_id" ]; do
  out=$("$AGENT_NODE_BIN" "$AGENT_BIN" media wait "$task_id" --since "$since")
  ec=$?
  last=$(printf '%s\n' "$out" | tail -1)
  since=$(printf '%s\n' "$last" |
    python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('nextSince',0))" 2>/dev/null)
...
- Do not invent claims, certifications, measurements, ingredients, or
  performance data.
- Use `"$AGENT_NODE_BIN" "$AGENT_BIN" media generate`; do not call provider APIs
  directly.
