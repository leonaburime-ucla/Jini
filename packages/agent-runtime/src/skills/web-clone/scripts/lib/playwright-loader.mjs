...
//      project — the normal fix inside the host platform, where these scripts are
//      staged under `.od-skills/<plugin>/scripts/` but run from the project
//      root);
//   3. AGENT_PLAYWRIGHT_PATH — an explicit package-dir escape hatch.
export function loadPlaywright() {
  ...
  const attempts = [
    ...
    () => {
      const p = process.env.AGENT_PLAYWRIGHT_PATH;
      if (!p) throw new Error("AGENT_PLAYWRIGHT_PATH unset");
  ...
  throw new Error(
    "Playwright not found. Fix (run in the project root, once per project):\n" +
      ...
      "automatically — no download needed). AGENT_PLAYWRIGHT_PATH=<playwright package dir> " +
