...
description: |
  ...
  the host platform browser-tab context, testing web apps, dogfooding the host platform
  previews, QA, bug hunts, or reviewing app quality. Prefer local the host platform
...
Use `agent-browser` for local the host platform preview validation: inspect rendered
...
For selected the host platform browser tabs and browser-use/browser-harness-style
...
5. If the selected the host platform context only provided a URL/title and no browser
...
Do not run the host platform's own daemon CLI as a browser automation tool. Commands
such as `od browser snapshot`, `daemon-cli.mjs browser snapshot`, or
`$AGENT_NODE_BIN $AGENT_BIN browser snapshot` are not valid browser tools; they can be
...
With the the host platform preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `the host platform`, current URL under `127.0.0.1:17573`,
visible the host platform UI text in the snapshot, and a screenshot at
