...
function promptArgvBudgetMessage(def: RuntimeAgentDef, bytes: number, limit: number): string {
  if (def.id === 'deepseek') {
    return (
...
function resolveArgvPromptBudget(maxPromptArgBytes: number, platform: NodeJS.Platform): number {
