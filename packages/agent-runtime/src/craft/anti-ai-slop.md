...
suited to auto-enforcement by a lint-artifact-style linter — failing an
...
"(guidance, not auto-checked)" so the contract with any linter stays
honest.
...
These are the patterns a linter should block at P0 (must-fix):

1. **Default Tailwind indigo as accent** — exactly `#6366f1`, `#4f46e5`,
   `#4338ca`, `#3730a3`, `#8b5cf6`, `#7c3aed`, `#a855f7`. The active
   design system provides an `--accent` token; use it. Indigo is the
   textbook AI tell. (Keep this list in sync with whatever constant
   your linter uses to flag a solid accent.)
...
4. **Sans-serif on display text when the seed binds a serif** — h1/h2
   must use the design system's declared display font, not a
   hardcoded Inter / Roboto / `system-ui`.
...
- **External placeholder image CDNs** (`unsplash.com`, `placehold.co`,
  `placekitten.com`, `picsum.photos`). Fragile and obvious. Use a
  local placeholder asset/class instead.
...
- **Sections without a stable data attribute or id** — comment/review
  tooling can't target them.
