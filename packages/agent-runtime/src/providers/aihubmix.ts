...
// Default base URL a caller can assume when its BYOK form leaves the field
// blank. Kept here as the single source of truth so a chat proxy, media
// renderer and connection test can all default to the same origin.
...
// unified OpenAI endpoint. Classify here; callers route in their own chat
...
// Catalogue ids vs wire names. A media registry may require globally-unique
// model ids across providers, but `gpt-image-1` / `dall-e-3` / `tts-1` are
// already owned by the `openai` provider in such a registry. So AIHubMix's
// models are registered with an `aihubmix-` prefix and mapped back to the
// real upstream name here. A plain prefix strip is the fallback so adding a
// new `aihubmix-<wire>` entry needs no edit here.
...
// lives under the host's `/api/v1` path, not the chat base's `/v1`, so the
// origin is derived from the configured base URL and the catalogue path
// appended.
...
// media-generation capabilities that belong to dedicated image/video/audio
...
// Aspect -> pixel size for a chat `generate_image` tool. Tuned for the
// default model (gpt-image-1, which accepts 1024x1024 / 1536x1024 /
// 1024x1536). A CLI/media renderer path may use a model-aware size resolver
// instead; this conservative table keeps a tool call from 400-ing on an
// unsupported size.
