...
Any one of these picked ad hoc risks becoming exactly the failure mode `docs/jini-port/extraction-plan.md` §7 and this repo's own architecture-review process exist to prevent: a kernel-adjacent decision made by whichever task happened to touch it first, rather than reviewed as a kernel-wide contract.
