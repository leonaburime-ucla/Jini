...
// (docs/jini-port/extraction-plan.md §4.3, §8 task 1).
//
...
//
//   1. `integrations/open-design/sync-ownership.manifest.json` maps upstream
//      OD daemon paths to `product-owned` | `delegated-to-jini`.
//   2. `scripts/patch-canary.ts` exports `transformUpstreamPath(path): string`
//      rewriting `apps/daemon/...` -> `integrations/open-design/daemon/...`,
...
//
// See `integrations/open-design/reference/fixtures/README.md` for why the
...
const manifestPath = path.join(repoRoot, 'integrations/open-design/sync-ownership.manifest.json');
const patchCanaryScript = path.join(repoRoot, 'scripts/patch-canary.ts');
const fixturePatch = path.join(
  repoRoot,
  'integrations/open-design/reference/fixtures/upstream-daemon-sample.patch',
);

const FIXTURE_UPSTREAM_PATH = 'apps/daemon/src/routes/health.ts';
const MIRRORED_PATH = 'integrations/open-design/daemon/src/routes/health.ts';
...
describe('milestone 1 gate O — sync-ownership manifest + patch canary', () => {
  ...
  it('applies a known upstream daemon patch cleanly via the path transform', () => {
    ...
    // mutates the real integrations/open-design tree.
