...
export interface DaemonStreamHandlers extends StreamHandlers {
  ...
  /**
   * A browser-surface agent tool (slice 2: `navigation.goto` / `ui.click`)
   * was dispatched toward this run's live browser session. Kept off
   * `AgentEvent`/`PersistedAgentEvent` — it is a side-effect trigger, not a
   * chat-rendered event, same precedent as `onToolInputDelta`.
   */
  onBrowserActionRequest?: (req: BrowserActionRequestSsePayload) => void;
}
...
export interface DaemonStreamOptions {
  ...
  // This browser tab's stable identity (getBrowserSessionId), so the
  // daemon's browser-action claim gate (pending-invocations.ts) prefers
  // routing this run's browser actions back to the tab that started it,
  // instead of whichever subscribed tab wins the broadcast race.
    ...
    for (let reconnects = 0; endStatus === null && reconnects < 5;) {
      ...
      while (true) {
        ...
        while ((idx = buf.indexOf('\n\n')) !== -1) {
          ...
          if (event.event === 'browser_action_request') {
            handlers.onBrowserActionRequest?.(event.data);
            continue;
          }
