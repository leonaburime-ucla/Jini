...
import { getBrowserSessionId } from '../providers/dom';
...
import type {
  ...
  BrandStatus,
  BrowserActionRequestSsePayload,
...
export function projectSplitClassName(
  ...
  if (workspaceFocused) return 'split split-focus';
  return leftPanelActive ? 'split split-left-panel' : 'split split-chat-overlay-mode';
}
...
export function projectSplitStyle(
  ...
  if (workspaceFocused || !leftPanelActive) return undefined;
  return {
...
export function ProjectView({
  ...
  // A fresh SSE attach (reload, second tab, remount) replays every buffered
  // event past cursor 0, including already-executed `browser_action_request`
  // events; this guards against re-navigating or re-clicking on replay.
  const handledBrowserActionInvocationIdsRef = useRef<Set<string>>(new Set());
  // Identifies this tab to the daemon's per-invocation claim gate. Two tabs
  // subscribed to the same run's SSE stream both receive every
  // `browser_action_request` broadcast; without a claim, both would execute
  // the same click/navigation independently. Backed by sessionStorage
  // (getBrowserSessionId) rather than a fresh per-mount UUID so the id stays
  // stable across a project switch (ProjectView remounts via its `key` prop)
  // — the daemon's claim-preference targeting (pending-invocations.ts) needs
  // "this tab", not "this particular ProjectView instance".
  const browserActionSessionIdRef = useRef<string>(getBrowserSessionId());
  ...
  const handleSend = useCallback(
    async (
      ...
      const handlers = {
        ...
        // Pure side effect, no chat bubble — same shape as `onToolInputDelta`.
        // Shared with useConversationChat's secondary loop (Side Chat tab +
        // GlobalAssistantHost) via executeBrowserActionRequest so a
        // workspace-scoped or side-chat run executes browser actions
        // identically to the main project chat.
        onBrowserActionRequest: (req: BrowserActionRequestSsePayload) => {
          executeBrowserActionRequest(
            req,
            browserActionSessionIdRef.current,
            handledBrowserActionInvocationIdsRef.current,
          );
        },
        onDone: (fullText = '') => {
      ...
      if (config.mode === 'daemon') {
        ...
        void streamViaDaemon({
          ...
          browserSessionId: browserActionSessionIdRef.current,
  ...
  useEffect(() => {
    if (projectActionsToast?.scope !== 'chat-pane') return;
    if (workspaceFocused || commentInspectorActive) return;
    setChatOverlayOpen(true);
  }, [commentInspectorActive, projectActionsToast?.scope, workspaceFocused]);
  ...
  const chatOverlayAvailable =
    !commentInspectorActive && Boolean(activeConversationId || conversationLoadError);
  const chatFabLabel = chatOverlayOpen ? 'Close chat' : 'Open chat';
    ...
    <div className="app">
      ...
      >
        <div
          className={[
            'split-chat-slot',
            !commentInspectorActive ? 'split-chat-slot--global' : '',
            chatOverlayOpen && !commentInspectorActive ? 'is-open' : '',
          ].filter(Boolean).join(' ')}
          hidden={workspaceFocused && commentInspectorActive}
        >
          {commentInspectorActive ? (
            <div
        ...
        {!workspaceFocused && leftInspectorActive ? (
      ...
      {chatOverlayAvailable ? (
        <button
          type="button"
          className={`project-chat-fab${chatOverlayOpen ? ' is-open' : ''}`}
          aria-label={chatFabLabel}
          title={chatFabLabel}
          aria-pressed={chatOverlayOpen}
          onClick={() => setChatOverlayOpen((open) => !open)}
        >
          <Icon name={chatOverlayOpen ? 'close' : 'message-circle'} size={20} />
        </button>
      ) : null}
