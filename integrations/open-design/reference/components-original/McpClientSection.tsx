 ...
 * `claude-mcp-json` / `acp-merge` / `opencode-env-content` /
 * `mimo-env-content` / `codex-config-override`, or undefined when no
 * native injection is wired yet).
 *
...
function McpAgentSupportBanner({ agents }: { agents: AgentInfo[] }) {
  ...
  // Codex is narrower still: `codex-config-override` forwards ONLY the
  // open-design-browser-actions bridge (see codexAgentDef.buildArgs), not
  // the user's other configured external MCP servers — Codex has no safe
  // config-file injection point for those yet.
  const renderNames = (list: AgentInfo[]) =>
    list
      .slice()
      .sort((a, b) => a.name.localeCompare(b.name))
      .map((a) => {
        if (a.externalMcpInjection === 'acp-merge') return `${a.name} (stdio only)`;
        if (a.externalMcpInjection === 'codex-config-override') return `${a.name} (browser tools only)`;
        return a.name;
      })
      .join(' · ');
