...
| `integrations/open-design` | Map OD logic to Jini interfaces | `ODChatAdapter`, `ODDataRoot` | `jini-*`, OD libraries | (Connects the two domains) | Open Design |
...
**Ledger Contract:** `docs/jini-port/tasks.json` is the committed source of truth.
...
**Ephemeral vs Committed:** `tasks.json`, validation scripts, and `decisions.md` are committed. Agent scratch files and active session logs (`docs/jini-port/sessions/<id>.md`) remain local/ephemeral until a task transitions out of `in_progress`, at which point the handoff note is committed.
...
2. `ledger-bootstrap` — create `docs/jini-port/{tasks.json,leases.json,sessions/,decisions.md,blockers.md}` + `project-runner` skeleton. Validation: `jini-next-task` runs.
