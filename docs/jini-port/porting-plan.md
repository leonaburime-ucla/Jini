...
project-runner/bin/jini-next-task
  Prints the next unblocked task from docs/jini-port/tasks.json.

project-runner/bin/jini-start-session <task-id> --agent codex|claude|local
  Creates docs/jini-port/sessions/<timestamp>-<task-id>.md and marks the task in_progress.
...
```txt
docs/jini-port/tasks.json
docs/jini-port/sessions/
docs/jini-port/decisions.md
docs/jini-port/blockers.md
docs/jini-port/source-branches.md
```
...
The important rule: a cloud agent should be able to open `docs/jini-port/tasks.json`, pick the next `pending` or resumable `in_progress` task, read the last session handoff, and continue without this chat.
...
2. Create `docs/jini-port/tasks.json`.
