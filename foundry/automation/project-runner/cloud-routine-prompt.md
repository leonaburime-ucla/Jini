...
(`trig_01BpadA61VBEnmgUvzZvo8nk`, see `foundry/docs/jini-port/milestone-1-red-spec.md` for
what it produced).
...
- `session_context.sources` supports exactly **one** `git_repository`, cloned at
  ...
  `foundry/docs/jini-port/od-reference-branches.md`) it clones itself, live, via Bash
...
- Lessons already folded into the prompt below: (1) a `red-spec` task's whole
  ...
  (2) explicitly point at `foundry/docs/jini-port/od-reference-branches.md` so a session
...
2. foundry/docs/jini-port/START-HERE.md and foundry/docs/jini-port/extraction-plan.md — the
   locked architecture and the numbered extraction task list (section 8).
3. foundry/automation/project-runner/README.md — the ledger you'll be driving.
4. foundry/docs/jini-port/od-reference-branches.md — if your claimed task touches
...
2. Before claiming or editing, record a dispatch preflight in the session
   ...
   - the exact consolidation-map row in
     `foundry/docs/jini-port/god-components-extraction-plan.md` covering this target
...
8. Commit everything (the code changes AND the updated
   foundry/automation/project-runner/ledger/* files) to the task branch and open a
