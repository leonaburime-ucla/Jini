...
`foundry/docs/jini-port/recon/r5-components-sweep.md`'s classification. Source files
are vendored at `foundry/integrations/open-design/reference/components-original/` —
...
- **Flat-group** (mechanical move + barrel export, no ports/hooks needed): for
  ...
  (`foundry/docs/jini-port/od-reference-branches.md`).
...
2. **Real `project-runner` WorkItems** — extend the ledger's DAG so `claim`
   ...
   This is real but not-yet-done engineering: `foundry/automation/project-runner/src/dag/
